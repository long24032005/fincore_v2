    ...
    const saveRecipientIfNeeded = async () => {
        ...
        try {
            ...
            const saveResult = await saveRecipient({
                userId: sender.$id,
                nickname: recipientNickname,
                transferType: recipientData.destinationType === 'wallet' ? 'wallet' : 'bank', // ✅ FIX: Use actual destination type
                recipientUserId: recipientData.userId,
                recipientWalletId: (recipientData as any).walletId, // NEW: from QR data
                recipientEmail: recipientData.email,
        ...
        try {
            ...
            if (method === 'wallet') {
                ...
                if (recipientData.destinationType === 'bank') {
                    ...
                    toast.success(
                        `✅ Payment Sent! $${transferAmount.toFixed(2)} deducted from wallet. Will arrive in recipient's bank in 1-3 days.`,
                        { duration: 6000 }
                    );
                    router.push('/');
                    ...
                    if (result && result.success) {
                        ...
                        toast.success(
                            `✅ Payment Successful! Sent $${transferAmount.toFixed(2)} to ${recipientData.name}. New balance: $${result.newBalance.toFixed(2)}`,
                            { duration: 5000 }
                        );
                ...
                if (recipientData.destinationType === 'wallet') {
                    ...
                    if (transaction) {
                        ...
                        toast.success(
                            `✅ Payment Sent! $${transferAmount.toFixed(2)} will be credited to ${recipientData.name}'s wallet once processed (1-3 days).`,
                            { duration: 6000 }
                        );
                        router.push('/');
                    } else {
